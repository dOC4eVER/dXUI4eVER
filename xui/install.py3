...
def install(rType="MAIN"):
    ...
    if os.path.exists("/tmp/install.sh"):
        printc("Installing Software - wait maybe slowly")
        os.system('bash /tmp/install.sh >/dev/null 2>&1')
        try: os.remove("/tmp/install.sh")
        except:
            pass
        return True
...
if __name__ == "__main__":
    ...
    if not rVersion in rVersions:
        printc("Unsupported Operating System, Works only on Ubuntu Server 20.04 and 22.04")
        sys.exit(1)
    printc("CLEAN MARIADB INSTALLATION", col.OKGREEN, 2)
		
    try: rType = input("  INSTALLATION TYPE [MAIN, LB]: ")
