...
rDownloadURL = {"main": "https://github.com/dOC4eVER/dXUI4eVER/raw/master/xui/install-main.sh",
                 "sub": "https://github.com/dOC4eVER/dXUI4eVER/raw/master/xui/install-sub.sh"}
...
rPackages = ["jq", "python3", "software-properties-common", "libmaxminddb0", "libmaxminddb-dev", "mmdb-bin", "libcurl4", "libgeoip-dev", "libxslt1-dev", "libonig-dev", "e2fsprogs", "wget", "mariadb-server", "v4l-utils", "mcrypt", "iptables-persistent", "libjpeg-dev", "libpng-dev", "xz-utils", "zip", "unzip"]
...
def mysql(rUsername, rPassword):
    ...
    printc("Configuring MariaDB wait...")
    rCreate = True
    ...
    os.system("service mariadb restart >/dev/null 2>&1")
    rExtra = ""
    os.system('mysql -u root%s -e "DROP DATABASE IF EXISTS xtream_iptvpro; CREATE DATABASE IF NOT EXISTS xtream_iptvpro;" 2> /dev/null' % rExtra)
    os.system("mysql -u root%s xtream_iptvpro < /home/xtreamcodes/iptv_xtream_codes/database.sql 2> /dev/null" % rExtra)
    # UPDATE V46 CREATE TABLE ODIN_BLOCKER
    os.system('mysql -u root%s -e "USE xtream_iptvpro; DROP TABLE IF EXISTS odin_blocker; CREATE TABLE odin_blocker (id int(11) NOT NULL AUTO_INCREMENT,timestamp datetime DEFAULT NULL,ip varchar(255) DEFAULT NULL,PRIMARY KEY (id));" 2> /dev/null' % rExtra)
    os.system('mysql -u root%s -e "USE xtream_iptvpro; UPDATE settings SET live_streaming_pass = \'%s\', unique_id = \'%s\', crypt_load_balancing = \'%s\';" 2> /dev/null' % ( rExtra, generate(20), generate(10), generate(20)))
    os.system('mysql -u root%s -e "USE xtream_iptvpro; REPLACE INTO streaming_servers (id, server_name, domain_name, server_ip, vpn_ip, ssh_password, ssh_port, diff_time_main, http_broadcast_port, total_clients, system_os, network_interface, latency, status, enable_geoip, geoip_countries, last_check_ago, can_delete, server_hardware, total_services, persistent_connections, rtmp_port, geoip_type, isp_names, isp_type, enable_isp, boost_fpm, http_ports_add, network_guaranteed_speed, https_broadcast_port, https_ports_add, whitelist_ips, watchdog_data, timeshift_only) VALUES (1, \'Main Server\', \'\', \'%s\', \'\', NULL, NULL, 0, 25461, 1000, \'%s\', \'eth0\', 0, 1, 0, \'\', 0, 0, \'{}\', 3, 0, 25462, \'low_priority\', \'\', \'low_priority\', 0, 1, \'\', 1000, 25463, \'\', \'[\"127.0.0.1\",\"\"]\', \'{}\', 0);" 2> /dev/null' % ( rExtra, getIP(), getVersion()))
...
def start():
    ...
    os.system("wget https://github.com/dOC4eVER/dXUI4eVER/raw/master/start_services.sh -qO /home/xtreamcodes/iptv_xtream_codes/start_services.sh")
    os.system("chmod 777 /home/xtreamcodes/iptv_xtream_codes/start_services.sh")
    os.system("wget -qO /tmp/install-bin-packages.sh https://github.com/dOC4eVER/dXUI4eVER/raw/master/xui/install-packages.sh >/dev/null 2>&1")
...
if __name__ == "__main__":
    ...
    printc("CLEAN MARIADB INSTALLATION maybe wait slowly +/- 30 minutes", col.OKGREEN, 2)
    #try:
    ...
    #rType.upper() = MAIN
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            ...
            printc("YOUR ADMIN DASHBOARD IS: http://%s:25500" % getIP())
            if rType.upper() == "MAIN":
